# PowerShell Script to set up Redis MCP server for Claude Desktop on Windows
# This script:
# 1. Checks for REDIS_URL in settings or .env file
...
$SettingsFile = Join-Path $RepoRoot "scripts\settings\redisSettings.env"
$DockerImage = "mcp/redis:latest"
$ContainerName = "redis-mcp-server"
...
# Step 1: Check for REDIS_URL in settings or .env file
Write-Host "Checking for Redis connection string..."

# Load settings from redisSettings.env if it exists
if (Test-Path $SettingsFile) {
    ...
    if ($settings.ContainsKey("REDIS_URL") -and -not [string]::IsNullOrWhiteSpace($settings["REDIS_URL"])) {
        $RedisUrl = $settings["REDIS_URL"]
        Write-Host "Using Redis URL from settings file" -ForegroundColor Cyan
    }
}

# Extract Redis URL from .env file if not already set from settings file
if ($RedisUrl -eq "redis://localhost:6379" -and (Test-Path $EnvFile)) {
    $envContent = Get-Content $EnvFile -Raw
    if ($envContent -match 'REDIS_URL=(.+)') {
        $url = $matches[1]
        if (-not [string]::IsNullOrWhiteSpace($url)) {
            $RedisUrl = $url
            Write-Host "Using Redis URL from .env file" -ForegroundColor Cyan
        }
    }
}

Write-Host "Using Redis URL: $RedisUrl" -ForegroundColor Green
...
if (-not $imageExists) {
    ...
    $dockerfilePath = Join-Path $RepoRoot "src\redis"
...
if (-not (Test-ContainerRunning -ContainerName $ContainerName)) {
    ...
    # For Docker on Windows, use host.docker.internal instead of localhost in the Redis URL
    $redisUrlForDocker = $RedisUrl -replace "localhost", "host.docker.internal"
    
    # Start Docker container in interactive mode with a pseudo-TTY
    Start-Process -FilePath "cmd.exe" -ArgumentList "/c", "docker run -it --name $ContainerName $DockerImage `"$redisUrlForDocker`"" -WindowStyle Hidden
...
if ((-not (Test-Path $ClaudeConfig)) -or ((Get-Content $ClaudeConfig -Raw).Trim() -eq "{}") -or ((Get-Content $ClaudeConfig -Raw).Trim() -eq "")) {
    ...
    Write-Host "✅ Created new Claude Desktop config with redis MCP server" -ForegroundColor Green
} else {
    ...
    try {
        ...
        $config.mcpServers | Add-Member -MemberType NoteProperty -Name "redis" -Value $serverConfig -Force
        
        # Save the updated config
        $config | ConvertTo-Json -Depth 10 | Set-Content $ClaudeConfig
        Write-Host "✅ Updated Claude Desktop config with redis MCP server" -ForegroundColor Green
    }
    catch {
...
Write-Host "`nRedis MCP server setup complete!" -ForegroundColor Green
Write-Host "You can now use Redis database access in Claude Desktop" -ForegroundColor Green
