# PowerShell Script to set up Time MCP server for Claude Desktop on Windows
# This script:
# 1. Loads settings from timeSettings.env
...
$SettingsFile = Join-Path $RepoRoot "scripts\settings\timeSettings.env"
$DockerImage = "mcp/time:latest"
$ContainerName = "time-mcp-server"
...
# Step 1: Load settings from timeSettings.env if it exists
if (Test-Path $SettingsFile) {
    ...
    if ($settings.ContainsKey("DEFAULT_TIMEZONE") -and -not [string]::IsNullOrWhiteSpace($settings["DEFAULT_TIMEZONE"])) {
        [Environment]::SetEnvironmentVariable("DEFAULT_TIMEZONE", $settings["DEFAULT_TIMEZONE"], "Process")
        Write-Host "Using custom default timezone: $($settings["DEFAULT_TIMEZONE"])" -ForegroundColor Cyan
    }
}
...
if (-not $imageExists) {
    ...
    $dockerfilePath = Join-Path $RepoRoot "src\time"
...
if (-not (Test-ContainerRunning -ContainerName $ContainerName)) {
    ...
    if ([Environment]::GetEnvironmentVariable("DEFAULT_TIMEZONE", "Process")) {
        $dockerRunCommand += " -e DEFAULT_TIMEZONE=$([Environment]::GetEnvironmentVariable('DEFAULT_TIMEZONE', 'Process'))"
    }
...
if ([Environment]::GetEnvironmentVariable("DEFAULT_TIMEZONE", "Process")) {
    $serverConfig.args += "-e"
    $serverConfig.args += "DEFAULT_TIMEZONE=$([Environment]::GetEnvironmentVariable('DEFAULT_TIMEZONE', 'Process'))"
}
...
if ((-not (Test-Path $ClaudeConfig)) -or ((Get-Content $ClaudeConfig -Raw).Trim() -eq "{}") -or ((Get-Content $ClaudeConfig -Raw).Trim() -eq "")) {
    ...
    Write-Host "✅ Created new Claude Desktop config with time MCP server" -ForegroundColor Green
} else {
    ...
    try {
        ...
        $config.mcpServers | Add-Member -MemberType NoteProperty -Name "time" -Value $serverConfig -Force
        
        # Save the updated config
        $config | ConvertTo-Json -Depth 10 | Set-Content $ClaudeConfig
        Write-Host "✅ Updated Claude Desktop config with time MCP server" -ForegroundColor Green
    }
    catch {
...
Write-Host "`nTime MCP server setup complete!" -ForegroundColor Green
Write-Host "You can now use time and timezone features in Claude Desktop" -ForegroundColor Green
