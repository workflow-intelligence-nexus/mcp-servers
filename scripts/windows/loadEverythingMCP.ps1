# PowerShell Script to set up Everything MCP server for Claude Desktop on Windows
# This script:
# 1. Loads settings from everythingSettings.env
...
$SettingsFile = Join-Path $RepoRoot "scripts\settings\everythingSettings.env"
$DockerImage = "mcp/everything:latest"
$ContainerName = "everything-mcp-server"
...
# Step 1: Load settings from everythingSettings.env if it exists
...
if (-not $imageExists) {
    ...
    $dockerfilePath = Join-Path $RepoRoot "src\everything"
...
if ((-not (Test-Path $ClaudeConfig)) -or ((Get-Content $ClaudeConfig -Raw).Trim() -eq "{}") -or ((Get-Content $ClaudeConfig -Raw).Trim() -eq "")) {
    ...
    $config = @{
        mcpServers = @{
            "everything" = $serverConfig
        }
    }
    
    # Save the config
    $config | ConvertTo-Json -Depth 10 | Set-Content $ClaudeConfig
    Write-Host "✅ Created new Claude Desktop config with everything MCP server" -ForegroundColor Green
} else {
    ...
    try {
        ...
        $config.mcpServers | Add-Member -MemberType NoteProperty -Name "everything" -Value $serverConfig -Force
        
        # Save the updated config
        $config | ConvertTo-Json -Depth 10 | Set-Content $ClaudeConfig
        Write-Host "✅ Updated Claude Desktop config with everything MCP server" -ForegroundColor Green
    }
    catch {
...
Write-Host "`nEverything MCP server setup complete!" -ForegroundColor Green
Write-Host "You can now use the Everything test server in Claude Desktop" -ForegroundColor Green
