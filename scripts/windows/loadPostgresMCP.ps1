# PowerShell Script to set up PostgreSQL MCP server for Claude Desktop on Windows
# This script:
# 1. Checks for POSTGRES_CONNECTION_STRING in settings or .env file
...
$SettingsFile = Join-Path $RepoRoot "scripts\settings\postgresSettings.env"
...
$PostgresConnectionString = "postgresql://localhost:5432/mydb"
...
# Step 1: Check for POSTGRES_CONNECTION_STRING in settings or .env file
Write-Host "Checking for PostgreSQL connection string..."

# Load settings from postgresSettings.env if it exists
if (Test-Path $SettingsFile) {
    ...
    if ($settings.ContainsKey("POSTGRES_CONNECTION_STRING") -and -not [string]::IsNullOrWhiteSpace($settings["POSTGRES_CONNECTION_STRING"])) {
        $PostgresConnectionString = $settings["POSTGRES_CONNECTION_STRING"]
        Write-Host "Using PostgreSQL connection string from settings file" -ForegroundColor Cyan
    }
}

# Extract connection string from .env file if not already set from settings file
if ($PostgresConnectionString -eq "postgresql://localhost:5432/mydb" -and (Test-Path $EnvFile)) {
    $envContent = Get-Content $EnvFile -Raw
    if ($envContent -match 'POSTGRES_CONNECTION_STRING=(.+)') {
        $ConnectionString = $matches[1]
        if (-not [string]::IsNullOrWhiteSpace($ConnectionString)) {
            $PostgresConnectionString = $ConnectionString
            Write-Host "Using PostgreSQL connection string from .env file" -ForegroundColor Cyan
        }
    }
}

Write-Host "Using PostgreSQL connection string: $PostgresConnectionString" -ForegroundColor Green
...
if (-not $imageExists) {
    ...
    $dockerfilePath = Join-Path $RepoRoot "src\postgres"
...
if (-not (Test-ContainerRunning -ContainerName $ContainerName)) {
    ...
    # For Docker on Windows, use host.docker.internal instead of localhost in the connection string
    $connectionStringForDocker = $PostgresConnectionString -replace "localhost", "host.docker.internal"
    
    Start-Process -FilePath "cmd.exe" -ArgumentList "/c", "docker run -it --name $ContainerName $DockerImage `"$connectionStringForDocker`"" -WindowStyle Hidden
...
if ((-not (Test-Path $ClaudeConfig)) -or ((Get-Content $ClaudeConfig -Raw).Trim() -eq "{}") -or ((Get-Content $ClaudeConfig -Raw).Trim() -eq "")) {
    ...
    Write-Host "✅ Created new Claude Desktop config with postgres MCP server" -ForegroundColor Green
} else {
    ...
    try {
        ...
        $config.mcpServers | Add-Member -MemberType NoteProperty -Name "postgres" -Value $serverConfig -Force
        
        # Save the updated config
        $config | ConvertTo-Json -Depth 10 | Set-Content $ClaudeConfig
        Write-Host "✅ Updated Claude Desktop config with postgres MCP server" -ForegroundColor Green
    }
    catch {
...
Write-Host "`nPostgreSQL MCP server setup complete!" -ForegroundColor Green
Write-Host "You can now use PostgreSQL database access in Claude Desktop" -ForegroundColor Green
