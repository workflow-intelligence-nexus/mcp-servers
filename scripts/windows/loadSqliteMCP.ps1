# PowerShell Script to set up SQLite MCP server for Claude Desktop on Windows
# This script:
# 1. Loads settings from sqliteSettings.env
...
$SettingsFile = Join-Path $RepoRoot "scripts\settings\sqliteSettings.env"
$DockerImage = "mcp/sqlite:latest"
$ContainerName = "sqlite-mcp-server"
$ClaudeConfigDir = Join-Path $env:APPDATA "Claude"
$ClaudeConfig = Join-Path $ClaudeConfigDir "claude_desktop_config.json"
$DatabasePath = Join-Path $RepoRoot "sqlite.db"
...
# Step 1: Load settings from sqliteSettings.env if it exists
if (Test-Path $SettingsFile) {
    ...
    if ($settings.ContainsKey("DATABASE_PATH") -and -not [string]::IsNullOrWhiteSpace($settings["DATABASE_PATH"])) {
        ...
        Write-Host "Using custom database path: $DatabasePath" -ForegroundColor Cyan
    }
}
...
if (-not (Test-Path $DatabaseDir)) {
    ...
    Write-Host "Created database directory at $DatabaseDir" -ForegroundColor Green
}
...
if (-not $imageExists) {
    ...
    $dockerfilePath = Join-Path $RepoRoot "src\sqlite"
...
if (-not (Test-ContainerRunning -ContainerName $ContainerName)) {
    ...
    $dockerRunCommand += " -v `"$($DatabasePath)`:/app/sqlite.db"
...
if ((-not (Test-Path $ClaudeConfig)) -or ((Get-Content $ClaudeConfig -Raw).Trim() -eq "{}") -or ((Get-Content $ClaudeConfig -Raw).Trim() -eq "")) {
    ...
    Write-Host "✅ Created new Claude Desktop config with sqlite MCP server" -ForegroundColor Green
} else {
    ...
    try {
        ...
        $config.mcpServers | Add-Member -MemberType NoteProperty -Name "sqlite" -Value $serverConfig -Force
        
        # Save the updated config
        $config | ConvertTo-Json -Depth 10 | Set-Content $ClaudeConfig
        Write-Host "✅ Updated Claude Desktop config with sqlite MCP server" -ForegroundColor Green
    }
    catch {
...
Write-Host "`nSQLite MCP server setup complete!" -ForegroundColor Green
Write-Host "You can now use SQLite database features in Claude Desktop" -ForegroundColor Green
