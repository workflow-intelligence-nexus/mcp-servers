# PowerShell Script to set up Sequential Thinking MCP server for Claude Desktop on Windows
# This script:
# 1. Loads settings from sequentialthinkingSettings.env
...
$SettingsFile = Join-Path $RepoRoot "scripts\settings\sequentialthinkingSettings.env"
$DockerImage = "mcp/sequentialthinking:latest"
$ContainerName = "sequentialthinking-mcp-server"
...
# Step 1: Load settings from sequentialthinkingSettings.env if it exists
...
if (-not $imageExists) {
    ...
    $dockerfilePath = Join-Path $RepoRoot "src\sequentialthinking"
...
if (-not (Test-ContainerRunning -ContainerName $ContainerName)) {
    ...
    Start-Process -FilePath "cmd.exe" -ArgumentList "/c", "docker run -it --name $ContainerName $DockerImage" -WindowStyle Hidden
...
if ((-not (Test-Path $ClaudeConfig)) -or ((Get-Content $ClaudeConfig -Raw).Trim() -eq "{}") -or ((Get-Content $ClaudeConfig -Raw).Trim() -eq "")) {
    ...
    $config = @{
        mcpServers = @{
            "sequentialthinking" = $serverConfig
        }
    }
    
    # Save the config
    $config | ConvertTo-Json -Depth 10 | Set-Content $ClaudeConfig
    Write-Host "✅ Created new Claude Desktop config with sequentialthinking MCP server" -ForegroundColor Green
} else {
    ...
    try {
        ...
        $config.mcpServers | Add-Member -MemberType NoteProperty -Name "sequentialthinking" -Value $serverConfig -Force
        
        # Save the updated config
        $config | ConvertTo-Json -Depth 10 | Set-Content $ClaudeConfig
        Write-Host "✅ Updated Claude Desktop config with sequentialthinking MCP server" -ForegroundColor Green
    }
    catch {
...
Write-Host "`nSequential Thinking MCP server setup complete!" -ForegroundColor Green
Write-Host "You can now use sequential thinking features in Claude Desktop" -ForegroundColor Green
